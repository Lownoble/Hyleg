...
void MX_GPIO_Init(void)
{
  ...
  GPIO_InitStruct.Pin = FootContact_L_Pin|FootContact_R_Pin;
...
{
	if(HAL_GPIO_ReadPin(GPIOA, FootContact_L_Pin) == GPIO_PIN_RESET){
		contact[0] = 1;
	}
	else{
		contact[0] = 0;
	}
	if(HAL_GPIO_ReadPin(GPIOA, FootContact_R_Pin) == GPIO_PIN_RESET){
		contact[1] = 1;
	}
	else{
		contact[1] = 0;
