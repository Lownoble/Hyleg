...
%% 导入数据
data1 = textread('./DATA/horizontal_0.0MPa_5kg.txt');
data2 = textread('./DATA/horizontal_0.2MPa_5kg.txt');
data3 = textread('./DATA/horizontal_0.3MPa_5kg.txt');
data4 = textread('./DATA/horizontal_0.4MPa_5kg.txt');
data5 = textread('./DATA/horizontal_0.5MPa_5kg.txt');
...
for i = 1:length(data1)
      %5kg j=17
%     if((i>22&&i<73) || (i>289&&i<339) || (i>553&&i<601))       data1(i,j) = data1(i,j)-36;      end
%     if((i>161 && i<200) || (i>425 && i<465))       data1(i,j) = data1(i,j)+36;    end
    if(data1(i,j)>5) data1(i,j) = data1(i,j)-36;      end
    sum1 = sum1+ data1(i,7)^2;
end
average1 = sqrt(sum1/length(data1));

for i = 1:length(data2)
    if(data2(i,22)>15)       data2(i,22) = data2(i,22)-36;end
...
t = i/100;
Z = zeros(600,1);
a = plot(t,Z,'k','LineWidth',2,'HandleVisibility','off');
a.Color(4) = 0.6;
hold on
plot(t,data1(i,j),'Color',[0.4 0.4 1],'LineWidth',1.2);
hold on
plot(t,data2(i,j),'Color',[0.2 0.6 0.8],'LineWidth',1.2);
hold on
plot(t,data3(i,j),'Color',[0.2 1 0.2],'LineWidth',1.2);
hold on
plot(t,data4(i,j),'Color',[0.8 0.6 0.2],'LineWidth',1.2);
hold on
plot(t,data5(i,j),'Color',[1 0.3 0.3],'LineWidth',1.2);
hold on
title("负载5KG")
xlabel("时间(s)")
ylabel("电流(A)")
