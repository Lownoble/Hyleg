...
BalanceCtrl::BalanceCtrl(BipedalRobot *robModel){
    Vec6 s;
    Vec6 w, u;
...
Vec32 BalanceCtrl::calF(Vec3 ddPcd, Vec3 dWbd, RotMat rotM, Vec32 feetPos2B, VecInt2 contact){
    ...
    _Fprev = _F;
    return vec6ToVec32(_F);
}

void BalanceCtrl::calMatrixA(Vec32 feetPos2B, RotMat rotM, VecInt2 contact){
    _A.block(0, 0, 3, 3) = I3;
    for(int i(0); i < 2; ++i){
...
void BalanceCtrl::calConstraints(VecInt2 contact){
    int contactLegNum = 0;
    for(int i(0); i<2; ++i){
        if(contact(i) == 1){
            contactLegNum += 1;
    ...
    _CI.resize(5*contactLegNum, 6);
    _ci0.resize(5*contactLegNum);
    _CE.resize(3*(2 - contactLegNum), 6);
    _ce0.resize(3*(2 - contactLegNum));
